...
// opening 
PImage startBg, startButton, waitButton;


// dialog
PFont words;//,skipWord;
...
Movie endMovie;
Minim minim;
AudioPlayer song, songOpen, songRun;
...
void setup(){
  ...
  startBg = loadImage("img/start.png");
  startButton = loadImage("img/startButton.png");
  waitButton = loadImage("img/waitButton.png");
  
  
  // dialog
  words = createFont("font/PingFang Regular_0.ttf",12);
  //skipWord = createFont("font/jackeyfont.ttg",20);
  ...
  skipWord = loadImage("img/skipWord.png");
...
void playMusic(String song){
   minim = new Minim(this);
   songRun = minim.loadFile(song);
   songRun.setVolume(0.2);
   songRun.play();
   songRun.loop();
}
...
void draw(){
  switch(gameState){
    case GAME_START:
        ...
        // music
        minim = new Minim(this);
        songOpen = minim.loadFile("openshort.mp3");
        songOpen.play();
        ...
        pushStyle();
        imageMode(CENTER);
        image(waitButton,width/2,height*3/5);
        
        if(mouseX >= 390 && mouseX <= 605 && mouseY >= 625 && mouseY <= 690){ 
          image(startButton,width/2,height*3/5);
          if(mousePressed){
    ...
    case GAME_DIALOG: // dialog
        ...
        //if(isPlay){
         ...
         image(skip,900,50); 
         image(skipWord,900,80,50,50);
         if(mouseX >= 905 && mouseX <= 950 && mouseY >= 60 && mouseY <= 100){
           pushStyle();
           tint(255,255,0);
           image(skip,900,50); 
           image(skipWord,900,80,50,50);
    ...
    if(!isPlayOver){
      minim = new Minim(this);
      songGameover = minim.loadFile("gameover.mp3");
      songGameover.play(100);
      isPlayOver = true;
    }
    ...
    if(isPlayOver){
      ...
      fill(#ffffff);
      if(mouseX >= 440 && mouseX <= 555 && mouseY >= 860 && mouseY <= 880){
        fill(#fff200);
        if(mousePressed){
          songGameover.close();
           mousePressed = false;
    ...
    if(isWin){
      ...
      if (endMovie.time() == endMovie.duration()) {
        song.close();
        isPlayMusic = false;
        gameState = GAME_START;
      }
